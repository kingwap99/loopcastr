...
## 播放順序：依首播日期（升冪／降冪）（2026-09-26）

原本只能照來源給的順序播（頻道的 `/videos` 是最新在前）。新增每個模式的 `sort`：

| 值 | 意思 |
|---|---|
| `source` | 來源順序（預設；頻道就是最新在前） |
| `date-asc` | 首播日期舊到新 |
| `date-desc` | 首播日期新到舊 |

實作在 `build_playlist.py --sort`，**順序是「先取 `video_limit` 支 → 過濾 `max_age_hours`
→ 才排序」**，所以「最近 24 小時、由舊到新播」是 `max_age_hours=24` ＋ `sort=date-asc` ✓。
沒有時間戳的影片一律排到最後（排不出來的不該插在中間）。

控制台「① 來源設定 → 更多設定」多一個下拉選單（值／顯示文字：
`source`＝來源順序、`date-asc`＝首播日期：舊→新、`date-desc`＝首播日期：新→舊）。
為此模式表單的欄位產生器補上 `choice` 型別（原本只會生 `<input>`）。

【實測】本機用臣心報報清單（3 支）：

    --sort date-asc   -> 2023/07/15 -> 2023/07/22 -> 2023/07/29
    --sort date-desc  -> 2023/07/29 -> 2023/07/22 -> 2023/07/15

【實測】`.22` 控制台把 test 模式設成 `date-asc` 後按「儲存這個模式」→
`/api/config` 讀到 `sort = date-asc` ✓；再按「只掃描來源」→ 母清單順序變成
`UjqD-XY7D3c(#50) → YlC65MH0xoc(#51) → Ig3vtqtXowY(#52)` ✓。

順手修掉兩個空值問題：模式的 `modes.json` 還沒有 `sort` 這個 key 時，
下拉選單會顯示空白（select 找不到對應選項），存檔又會把空字串寫進去 ——
空字串會讓 `--sort` 直接失敗。現在選單會退回第一個選項，
而 `choice` 欄位若是空值就不寫入（讓程式用它自己的預設）。
...
## 裝在 clone 裡時，`git pull` 不會更新正在跑的程式（2026-09-24）

接續上一則：在 `.41` 拉了新版、重跑建置，日誌卻顯示

    list has 30 segments, 30 present, 0 missing (30 skipped by fingerprint)

—— 指紋完全沒變，代表**跑的還是舊程式**。原因是 `.41` 是「裝在 clone 裡」：`git pull` 更新的是
`src/*`（版控追蹤的那份），而服務與指令實際執行的是安裝目錄根目錄那份**攤平的複本**，那份只有
`install.sh` 會更新。

兩件修正：

1. README 與 manual 的安裝段落加上「**升級＝`git pull` ＋ 再跑一次 `./install.sh`**」，並說明
   pull 只動 `src/`。
