...
- 🔶 `docs/manual.md` 目前是「操作手冊 ＋ 逐日更新紀錄」混在一起（38 KB），要拆成手冊與
  變更紀錄（這份實測紀錄其實是 repo 最有價值的內容）
...
另外 `docs/manual.md` 已經 38 KB、45 個章節，内容是「操作手冊 ＋ 逐日更新紀錄」混在一起。開源的話建議拆成 `docs/manual.md`（現況，不含歷史）與 `docs/changelog.md`（實測紀錄，這份反而是最有價值的內容）。
