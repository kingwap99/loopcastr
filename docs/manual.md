...
2026-09-16 更新。這一版把「可部署的程式」補齊，並且把本輪實測出來的架構決定寫進來。
...
| `src/settings.json` | 通用設定（畫質、版面、淡化、黑尾門檻）。程式讀它當**預設值**，命令列可覆寫 |
| `src/modes.json` | 播出模式定義：來源頻道、長度上限、shorts 池、掃描頻率 |
| `playlist.example.json` | 母清單**範例**（3 筆假 id）。實際的 `playlist.json` 用 `build_playlist.py` 產生，已列入 `.gitignore` |
| `mediamtx.example.yml` | MediaMTX 範例設定。刻意用**路徑白名單**（只開 `live/main`），不是全開 |
| `src/build_local_content.py` | 把母清單的影片抓成本機 `media/<id>.mp4`，並產生 `playlist-local.json` |
| `src/build_transitions.py` | 產生每集專屬過場（QR 指向該集），可吃 shorts 池輪播 |
| `src/modes.json` | 播出模式定義：來源頻道、長度上限、shorts 池、掃描頻率都在這裡 |
...
| `src/relay.py` | 聯播／多來源接力引擎（takeover 零斷點換手、看門狗、URL 生命週期） |
| `src/gapwatch.py` | 獨立驗收觀測器：輪詢 MediaMTX API，報「接收端離線」與 `bytesReceived` 零成長區間 |
| `src/healthcheck.py` | 健康檢查（每 60 秒）：ready、有讀者、流量有成長；狀態變化才告警 |
| `src/loopwatch.py` | 循環邊界觀測：量繞回清單開頭那一刻有沒有縫 |
| `src/wmtext.py`／`make_qr_png.py` | 浮水印文字與 QR Code 的 PNG 產生（機器上沒有 freetype，所以自己畫） |
| `launchd/*.plist` | 服務定義**模板**：`__HOME__`／`__USER__` 由 `install.sh` 代入 |
| `launchd/com.ytpl.playout.plist` | 播出端常駐服務 |
| `launchd/com.ytpl.publish.plist` | 推流端常駐服務 |
| `launchd/com.ytpl.mediamtx.plist` | 媒體樞紐常駐服務（含 8192 fd 的 ResourceLimits） |
| `launchd/com.ytpl.health.plist` | 健康監控常駐服務（以 root 執行，才能 kickstart system domain） |

## 為什麼播出端用 concat 而不是接力（本輪實測）
...
| 動作 | 建置／只掃描／建置並切換、重建 concat、檢查缺檔。背景執行並回報進度（建置可能數十分鐘） |
